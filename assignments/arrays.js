...
let lastCar = 0;
console.log(`Final car in inventory is a ${inventory[inventory.length - 1].car_make} ${inventory[inventory.length - 1].car_model} `);
...
let oldCars = carYears.filter(carsYear => carsYear < 2000);
console.log(oldCars.length); 

// ==== Challenge 6 ====
// A buyer is interested in seeing only BMW and Audi cars within the inventory.  Return an array that only contains BMW and Audi cars.  Once you have populated the BMWAndAudi array, use JSON.stringify() to show the results of the array in the console.
let BMWAndAudi = inventory.filter(car => car.car_make.includes('Audi') || car.car_make.includes('BMW') );
